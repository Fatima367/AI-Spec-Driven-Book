...
def test_chat_with_greeting(mock_services):
    """Test chat interaction with a greeting query."""
    mock_qdrant_client, mock_get_embeddings, mock_llm_client = mock_services
    # Mock empty search results to test greeting handling
    mock_qdrant_client.query.return_value.points = []
    ...
    assert response.status_code == 200
    json_response = response.json()
    assert "Hello! I'm your Physical AI & Humanoid Robotics book assistant" in json_response["answer"]
    assert len(json_response["citations"]) == 0
    # LLM should not be called for greetings
    mock_llm_client.chat.completions.create.assert_not_called()


def test_chat_with_non_book_related_query(mock_services):
    """Test chat interaction with a non-book-related query."""
    mock_qdrant_client, mock_get_embeddings, mock_llm_client = mock_services
    # Mock empty search results to test query filtering
    mock_qdrant_client.query.return_value.points = []
    ...
    assert response.status_code == 200
    json_response = response.json()
    assert "I apologize, but I can only respond to greetings and queries related to Physical AI & Humanoid Robotics content" in json_response["answer"]
    assert len(json_response["citations"]) == 0
    # LLM should not be called for non-book-related queries
    mock_llm_client.chat.completions.create.assert_not_called()


def test_chat_with_book_related_query_no_context(mock_services):
    """Test chat interaction with a book-related query but no context found."""
    mock_qdrant_client, mock_get_embeddings, mock_llm_client = mock_services
    # Mock empty search results to test query filtering
    mock_qdrant_client.query.return_value.points = []
    ...
    response = client.post(
        "/api/v1/chat",
        json={"query": "What is humanoid robotics?"}
    )

    assert response.status_code == 200
    json_response = response.json()
    assert "I apologize, but I couldn't find any relevant information in the book" in json_response["answer"]
    assert len(json_response["citations"]) == 0
    # LLM should not be called when no context is found
    mock_llm_client.chat.completions.create.assert_not_called()
